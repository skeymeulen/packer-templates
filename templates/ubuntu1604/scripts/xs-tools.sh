...
select_pkgs_rhel()
{
	...
	rhel4|centos4)
		...
		if [ -f "${DATADIR}/rhel4x/versions" ] ; then
		    source ${DATADIR}/rhel4x/versions
		    KERNEL="${DATADIR}/rhel4x/${RHEL4X_KERNEL_PKG_FILE}"
		    if [ ${os_minorver} -lt 5 ] ; then
			    MKINITRD="${DATADIR}/rhel4x/mkinitrd-${RHEL4X_MKINITRD_PKG_VERSION}.i386.rpm"
		    fi
		    REQUIRE_XENBLK_KO=yes
		    REQUIRE_XENNET_KO=yes
		else
		    echo "Warning: Kernel package not found in ${DATADIR}/rhel4x"
		fi
	    fi
	    ;;
	rhel5|centos5|oracle5|scientific5)
	    ...
	    ;;
	rhel6|centos6|oracle6|scientific6)
	    # No additional kernel package
	    ;;
	rhel7|centos7|oracle7|scientific7)
	    # No additional kernel package
	    ;;
	rhel3|fedora*)
...
select_pkgs_debian()
{
	...
	debian:6:*) DISTRO=squeeze ;;
	debian:7:*) DISTRO=wheezy ;;
	debian:8:*) DISTRO=jessie ;;
	debian:testing:*|debian:unstable:*) DISTRO=${os_majorver} ;;

	ubuntu:10:04) DISTRO=lucid ;;
	ubuntu:10:10) DISTRO=maverick ;;
	ubuntu:11:04) DISTRO=natty ;;
	ubuntu:11:10) DISTRO=oneiric ;;
	ubuntu:12:04) DISTRO=precise ;;
	ubuntu:12:10) DISTRO=quantal ;;
	ubuntu:13:04) DISTRO=raring ;;
	ubuntu:13:10) DISTRO=saucy ;;
	ubuntu:14:04) DISTRO=trusty ;;
	ubuntu:1[456]:*) DISTRO=future_ubuntu ;;
    esac
    ...
    case ${DISTRO} in
	squeeze|wheezy|jessie) # No additional packages required
	    ;;
	testing|unstable)
	    ;;
	lucid|maverick|natty|oneiric|precise|quantal|raring|saucy|trusty|future_ubuntu)
	    ;;
	*)
	    failure "Unknown Debian variant \"${os_majorver}\" \"${DISTRO}\""
	    ;;
    esac
...
update_arp_notify_configuration()
{
    ...
    if [ ! -e "${file}" ] ; then
	echo "Cannot update arp_notify configuration as no ${file} present" >&2
	return 1
    fi
...
case "${os_distro}" in
    ...
    scientific)                select_pkgs_rhel ;;
    sles|"suse linux")         select_pkgs_sles ;;
    debian|ubuntu)             select_pkgs_debian ;;
...
if update_arp_notify_required ; then
    echo "  * update arp_notify sysctl.conf."
    update_arp_notify=1
else
    update_arp_notify=0
fi
